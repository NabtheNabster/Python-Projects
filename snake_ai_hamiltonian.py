...
pygame.display.set_caption("Snake AI Hamiltonian")
...
num_runs = 1
current_run = 0
# --- Fixed Hamiltonian Cycle Logic ---
hamilton_path = []

# 1. Down the first column (x=0)
for y in range(rows):
    hamilton_path.append([0, y])

# 2. S-curve through the rest (x=1 to cols-1)
for x in range(1, cols):
    if x % 2 != 0: # Odd columns go Up
        for y in range(rows - 1, 0, -1):
            hamilton_path.append([x, y])
    else:          # Even columns go Down
        for y in range(1, rows):
            hamilton_path.append([x, y])

# 3. Back across the top row to close the loop
for x in range(cols - 1, 0, -1):
    hamilton_path.append([x, 0])

# Precompute lookup
hamilton_index = { (x, y): i for i, (x, y) in enumerate(hamilton_path) }
...
while running:
    ...
    if current_run < num_runs:
        ...
        head = snake_body[0]
            ...
            # --- AI logic (greedy with Hamiltonian fallback) ---
        head = snake_body[0]
        idx = hamilton_index.get((head[0], head[1]), 0)
        next_cell = hamilton_path[(idx + 1) % len(hamilton_path)]
